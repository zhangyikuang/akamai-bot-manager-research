...
![目标请求的第一个栈](images/04-first-stack-ehwb.png)

从第一栈逐层上跳，每层看局部变量，最终在明文组装处停住——Scope 里是一个 82 元素的键值对数组，这就是第一阶段 `sensor_data` 的明文：

![逐层入栈到明文组装](images/08-stack-step2.png)
...
2. 真正被校验的只有 `bm_s`；
3. `bm_s` 的长度是服务端对长报的**判定编码**——长报评分通过，响应直接把它置到可用档位并放行页面；没通过就停在低档，再导航就被拒。
...
![当时的逐项差异清单](images/13-remaining-diff.png)
...
`me` 轨迹不是"非空"就行。40 个事件挤在一个同步循环里，时间列全是 0~1ms，单条请求看不出问题，跨请求一比对就是明确的机器特征。
...
- 两段脚本在浏览器里共享同一个 window、同一份 cookie、同一个时钟。拆成两个进程跑，状态就断了；必须放进同一个 VM 按序执行，并且每封报文发出后把服务端响应的状态回灌给还在运行的脚本；
